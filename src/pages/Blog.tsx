        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center mb-8">
            <div className="inline-flex rounded-md shadow-sm">
              <Button variant={selectedType === null ? "default" : "outline"} className="rounded-l-md rounded-r-none" onClick={() => setSelectedType(null)}>
                All
              </Button>
              <Button variant={selectedType === "publication" ? "default" : "outline"} className="rounded-none border-x-0" onClick={() => setSelectedType("publication")}>
                <BookOpen className="mr-2 h-4 w-4" />
                Publications
              </Button>
              <Button variant={selectedType === "blog" ? "default" : "outline"} className="rounded-r-md rounded-l-none" onClick={() => setSelectedType("blog")}>
                  ...
                  <AnimatedCard className="h-full">
                    <div className="p-6 flex flex-col h-full">
                      <div className="mb-4 flex items-center justify-between">
                        <span className="bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 px-2 py-1 rounded text-xs">{post.category}</span>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {post.tags.map(tag => (
                          <span key={tag} className="bg-secondary/50 px-2 py-1 rounded text-xs">#{tag}</span>
                        ))}
                      </div>
              ...
              .map((post, index) => (
                <motion.div key={post.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                  <AnimatedCard className="h-full">
                    <div className="p-6 flex flex-col h-full">
                      <div className="mb-4 flex items-center justify-between">
                        <span className="bg-primary/20 text-primary px-2 py-1 rounded text-xs">{post.category}</span>
                        <div className="flex items-center text-xs text-muted-foreground">
                          <Clock className="h-3 w-3 mr-1" />
                          <span>{post.readTime} read</span>
                        </div>
